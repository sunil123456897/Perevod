...
class DatabaseManager:
    ...
    def restore_term(self, quarantined_term_id: int):
        with self.session_scope() as s:
            term_to_restore = (
                s.query(QuarantinedTerm)
    ...
    def delete_from_quarantine(self, quarantined_term_id: int):
        with self.session_scope() as s:
            term_to_delete = (
                s.query(QuarantinedTerm)
                .filter_by(id=quarantined_term_id, project_id=self.project_id)
    ...
    def get_dictionary_proposals(self):
        with self.session_scope() as s:
            return { p.english_term: {"russian_term": p.russian_term, "category": p.category, "confidence": p.confidence} for p in s.query(DictionaryProposal).filter_by(project_id=self.project_id).all() }
