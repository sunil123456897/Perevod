...
from Perevod.utils.llm import clean_translation_output, tool_translate_chunk
import pymorphy3
...
def tool_translate_chapter_logic(eng_text: str, title: str, settings: Dict[str, Any], db_manager, kb_manager, model) -> str:
    ...
    if kb_manager and kb_manager.collection:
        try:
            query_results = kb_manager.query(query_texts=[eng_text], n_results=settings.get('kb_n_results', 3))
            if query_results and query_results['documents']:
                # Flatten the list of lists into a single list of documents
                documents = [doc for sublist in query_results['documents'] for doc in sublist]
                relevant_context = "\n\nRelevant Context from Knowledge Base:\n---\n" + "\n---\n".join(documents) + "\n---"
                logger.info(f"Найдено {len(documents)} релевантных документов из Базы Знаний для главы '{title}'.")
        except Exception as e:
            logger.warning(f"Ошибка при запросе к Базе Знаний для главы '{title}': {e}")

    # Construct the prompt, incorporating the relevant context
    base_prompt = f"""Translate the following English text from chapter '{title}' into Russian. Use the provided dictionary and knowledge base for consistency. Focus on maintaining the original meaning, tone, and style.
