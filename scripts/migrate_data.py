...
def migrate_project(project_name, project_config, base_dir):
    ...
    api_key = project_config.get("api_key") or settings.GOOGLE_API_KEY
    if not api_key:
        print(
            f"!!! ОШИБКА: API ключ не найден для проекта '{project_name}'. Пропускаем."
        )
        return
...
def main():
    ...
    print(
        "Старые JSON-файлы внутри папок проектов в '_project_files' также можно удалить после проверки."
    )
